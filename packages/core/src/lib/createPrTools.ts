...
export function createPrTools(workspace: string): ToolDefinition[] {
  return [
    ...
    defineJsonTool("diff_list_files", "List files changed in preview diff", Type.Object({ previewDiffId: Type.Optional(Type.Number()) }), async (params) => {
      ...
      return Promise.all(files.filter((file) => file.endsWith("/meta.json")).sort().map((file) => readJson(join(filesRoot, file))));
    }),
